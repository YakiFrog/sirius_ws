...
class Nav2GoalClient(Node):
    def __init__(self, yaml_file = "move_goal.yaml"):
        ...
        count = int(yaml_data["move_goal"]["ros_parameters"]["count"])
        self.stop = int(yaml_data["move_goal"]["ros_parameters"]["stop"])
        self.detect = int(yaml_data["move_goal"]["ros_parameters"]["detect"])
        self.change = yaml_data["move_goal"]["ros_parameters"]["change"]
        self.waypoint_file = yaml_data["move_goal"]["ros_parameters"]["waypoint_file"]
        
        self.action_client_ = ActionClient(self, NavigateToPose, 'navigate_to_pose')
        
        if self.stop:
            self.stop_list = yaml_data["move_goal"]["ros_parameters"]["stop_list"]
        else:
            self.stop_list = []
        ...
        if self.detect:
            ...
            self.detect_list = yaml_data["move_goal"]["ros_parameters"]["detect_list"]
        else:
            self.detect_list = []
